...
class PreActBlock_conv_Q(nn.Module):
  ...
  def __init__(self, wbit, abit, clip_val, in_planes, out_planes, stride=1):
  ...
  def forward(self, x):
    out = self.act_q(F.relu(self.bn0(x)), self.clip_val)
    ...
    out = self.conv0(out)
    out = self.act_q(F.relu(self.bn1(out)), self.clip_val)
    out = self.conv1(out)
    out += shortcut
    return out
...
class PreActResNet(nn.Module):
  def __init__(self, block, num_units, wbit, abit, clip_val, num_classes):
    ...
    for stride, channel in zip(strides, channels):
      self.layers.append(block(wbit, abit, clip_val, in_planes, channel, stride))
      in_planes = channel
...
def resnet20(wbits, abits, clip_val, num_classes=10):
  return PreActResNet(PreActBlock_conv_Q, [3, 3, 3], wbits, abits, clip_val,  num_classes=num_classes)


def resnet56(wbits, abits, clip_val, num_classes=10):
  return PreActResNet(PreActBlock_conv_Q, [9, 9, 9], wbits, abits, clip_val, num_classes=num_classes)
